...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 uppercase font-semibold">Aguard. Autorizacao</p>
            <p class="text-2xl font-bold text-blue-500 mt-1">{{ $orders->where('status', \App\Enums\ServiceOrderStatus::AWAITING_AUTHORIZATION)->count() }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 uppercase font-semibold">Em Andamento</p>
            <p class="text-2xl font-bold text-purple-500 mt-1">{{ $orders->whereIn('status', [\App\Enums\ServiceOrderStatus::AUTHORIZED, \App\Enums\ServiceOrderStatus::IN_PROGRESS])->count() }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 uppercase font-semibold">Aguard. Aprovacao</p>
            <p class="text-2xl font-bold text-yellow-500 mt-1">{{ $orders->where('status', \App\Enums\ServiceOrderStatus::AWAITING_APPROVAL)->count() }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 uppercase font-semibold">Concluidas</p>
            <p class="text-2xl font-bold text-green-500 mt-1">{{ $orders->whereIn('status', [\App\Enums\ServiceOrderStatus::COMPLETED, \App\Enums\ServiceOrderStatus::INVOICED, \App\Enums\ServiceOrderStatus::CLOSED])->count() }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div class="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                <div class="p-5">
                    <div class="flex items-start justify-between">
                        ...
                        <div class="text-right">
                            @if($order->customer_charge > 0)
                                ...
                                <p class="text-lg font-bold text-red-500">R$ {{ number_format($order->customer_charge, 2, ',', '.') }}</p>
                            @else
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        {{-- Botão AUTORIZAR --}}
                        @if($order->status === \App\Enums\ServiceOrderStatus::AWAITING_AUTHORIZATION && !$order->isAuthorized())
                            <a href="{{ route('os.signature.show', $order->id) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-blue-500 px-3 py-2 text-xs font-bold text-white hover:bg-blue-600 transition">
                        ...
                        {{-- Botão APROVAR --}}
                        @if($order->status === \App\Enums\ServiceOrderStatus::AWAITING_APPROVAL && !$order->isApproved())
                            <a href="{{ route('os.signature.show', $order->id) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-green-500 px-3 py-2 text-xs font-bold text-white hover:bg-green-600 transition">
                        ...
                        @if($order->isAuthorized())
                            <span class="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 border border-blue-200">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                Autorizada {{ $order->authorization_signed_at?->format('d/m') }}
                            </span>
                        @endif
                        ...
                        @if($order->isApproved())
                            <span class="inline-flex items-center gap-1.5 rounded-lg bg-green-50 px-3 py-2 text-xs font-semibold text-green-700 border border-green-200">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                Aprovada {{ $order->completion_signed_at?->format('d/m') }}
                            </span>
                        @endif

                        @if($order->isInvoiced())
                            <span class="inline-flex items-center gap-1.5 rounded-lg bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 border border-red-200">
                                💰 Faturada
                            </span>
                        @endif
                    </div>
