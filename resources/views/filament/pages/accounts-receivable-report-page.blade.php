        ...
        <tbody>
            @forelse($records as $record)
                @php
                    ...
                    // Placa do veículo (via invoice->contract->vehicle ou notes)
                    $plate = null;
                    ...
                    // Período da reserva (via notes)
                    $period = null;
                    if ($record->invoice && $record->invoice->notes && preg_match('/Periodo:\s*(.+?)(?:\n|$)/i', $record->invoice->notes, $mp)) {
                        $period = trim($mp[1]);
                    }
                @endphp
                <tr>
                    ...
                    <td class="col-fatura" style="color:#e5e7eb;">{{ $record->invoice->invoice_number ?? '—' }}</td>
            ...
            @empty
                <tr><td colspan="10" class="rpt-empty">Nenhuma conta a receber encontrada no período selecionado</td></tr>
            @endforelse
        </tbody>
    </table>
    </div>
